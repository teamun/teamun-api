...
exports.updateForAdmin = function(req, res) {
  ...
  if (activityGroup.numLimit) {
    activityGroup.checkAndUpdateAvailablePlaces()
      .then(function() {
        activityGroup.safeSave()
          .then(function(safeSaveResult) {
            console.log(safeSaveResult);
            return res.json(safeSaveResult);
          })
          .fail(function(safeSaveErr) {
            return res.json(safeSaveErr);
          });
      });
  } else {
    activityGroup.safeSave()
